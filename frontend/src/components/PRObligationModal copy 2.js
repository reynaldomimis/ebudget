...
const PRObligationModal = ({ isOpen, onClose, onSubmit, selectedActivity }) => {
  ...
  const validateForm = () => {
    ...
    // Check if PR Amount is greater than Obligation Amount
    if (
      ...
      toast.error(
        `Invalid Obligation Amount: Amount must be greater than the PR amount (${formatPHP(
    ...
    if (amount > totalAllocation) {
      ...
      Allocation: ${formatPHP(totalAllocation)}`,
      );
      return false;
    }
    return true;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
          ...
          if (isSuccess) {
            if (selectedRow?.PRNO) {
              try {
                ...
                console.error(
                  "Failed to update PR unobligated amount:",
                  prError,
                );
      ...
      // ======= RESET FORM =======
      const nextFormType = selectedRow
        ? "PR"
        : formData.type === "Obligation"
          ? "Obligation"
          : "PR";
  ...
  const handleProceed = async (row) => {
    ...
    const amountPR = parsePHP(row.OBLIGATED);
    if (amountPR > 0) {
          ...
          <div className="flex-[0_0_30%] p-4 border-b lg:border-r border-gray-200 overflow-y-auto">
            ...
            <div className="mb-2 text-xs space-y-1">
              ...
              <div>
                ...
                {formatPHP(selectedActivity?.total_amount)}
              </div>
              <div>
                <span className="font-bold"> Balance: </span>{" "}
                {formatPHP(totalAllocation)}
              </div>
            </div>
