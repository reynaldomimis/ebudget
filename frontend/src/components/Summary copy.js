...
const Summary = () => {
  ...
  const fetchAllData = async () => {
    setIsLoading(true);
    try {
      const [prSoRes, oblRes, expenseRes] = await Promise.all([
        prSoAPI.getActivity(),
      ...
      // Unique expense items
      const items = expenseRes.data.data || [];
      const trimmedUnique = [...new Set(items.map((s) => s.trim()))];
      setExpenseItems(trimmedUnique);
    } catch (err) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        {/* <div className="bg-white rounded-lg shadow p-6">
          ...
          <p className="text-2xl font-bold text-purple-600">
            {formatPHP(filteredPrSoTotals.balancePR)}
          </p>
        </div> */}
      </div>
    </div>
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="bg-white rounded-lg shadow p-8 flex-1 overflow-auto flex flex-col items-center">
        ...
        {selectedView === "SUMMARY" &&
          (selectedDivision || selectedExpenseItem) && (
            <>
              <ActivitiesTable
                division={selectedDivision}
                papType={selectedPapKey}
